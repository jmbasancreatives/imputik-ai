...
function LessonView({ lesson, state, update, close, select }: { lesson: Lesson; state: AppState; update: (p: Partial<AppState>) => void; close: () => void; select: (l: Lesson) => void }) {
  ...
  const videoDone = state.videoAssessmentAnswers[lesson.id] !== undefined
  const videoChosenIdx = state.videoAssessmentAnswers[lesson.id]
  ...
  const chooseVideo = (index: number) => {
    ...
    update({ videoAssessmentAnswers: { ...state.videoAssessmentAnswers, [lesson.id]: index }, videoAssessmentScores: { ...state.videoAssessmentScores, [lesson.id]: correct ? 100 : 0 } })
  }
