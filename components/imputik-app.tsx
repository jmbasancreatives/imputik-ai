...
function LessonView({ lesson, state, update, close, select }: { lesson: Lesson; state: AppState; update: (p: Partial<AppState>) => void; close: () => void; select: (l: Lesson) => void }) {
  ...
  const videoAssessmentAnswers = state.videoAssessmentAnswers || {}
  const videoChosenIdx = videoAssessmentAnswers[lesson.id]
  const videoDone = videoChosenIdx !== undefined
...
function SettingsView({ state, update }: { state: AppState; update: (p: Partial<AppState>) => void }) {
